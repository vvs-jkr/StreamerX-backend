...
export const MESSAGES = {
	welcome:
		`<b>👋 Добро пожаловать в StreamerX Bot!</b>\n\n` +
		`Чтобы получать уведомления и улучшить ваш опыт использования платформы, давайте свяжем ваш Telegram аккаунт с StreamerX.\n\n` +
		`Нажмите на кнопку ниже и перейдите в раздел <b>Уведомления</b>, чтобы завершить настройку.`,
	authSuccess: `🎉 Вы успешно авторизовались и Telegram аккаунт связан с StreamerX!\n\n`,
	...
	follows: (user: User) =>
		`📺 <a href="https://vs-dev-streamerx.ru/${user.username}">${user.username}</a>`,
	resetPassword: (token: string, metadata: SessionMetadata) =>
		`<b>🔒 Сброс пароля</b>\n\n` +
		`Вы запросили сброс пароля для вашей учетной записи на платформе <b>StreamerX</b>.\n\n` +
		`Чтобы создать новый пароль, пожалуйста, перейдите по следующей ссылке:\n\n` +
		`<b><a href="https://vs-dev-streamerx.ru/account/recovery/${token}">Сбросить пароль</a></b>\n\n` +
		...
		`Спасибо за использование <b>StreamerX</b>! 🚀`,
	deactivate: (token: string, metadata: SessionMetadata) =>
		...
		`Спасибо, что пользуетесь <b>StreamerX</b>! Мы всегда рады видеть вас на нашей платформе и надеемся, что вы останетесь с нами. 🚀\n\n` +
		`С уважением,\n` +
		`Команда StreamerX`,
	accountDeleted:
		...
		`<b><a href="https://vs-dev-streamerx.ru/account/create">Зарегистрироваться на StreamerX</a></b>\n\n` +
	...
	streamStart: (channel: User) =>
		...
		`Смотрите здесь: <a href="https://vs-dev-streamerx.ru/${channel.username}">Перейти к трансляции</a>`,
	newFollowing: (follower: User, followersCount: number) =>
		`<b>У вас новый подписчик!</b>\n\nЭто пользователь <a href="https://vs-dev-streamer.ru/${follower.username}">${follower.displayName}</a>\n\nИтоговое количество подписчиков на вашем канале: ${followersCount}`
	// newSponsorship: (plan: SponsorshipPlan, sponsor: User) =>
	// 	`<b>🎉 Новый спонсор!</b>\n\n` +
	// 	`Вы получили новое спонсорство на план <b>${plan.title}</b>.\n` +
	// 	`💰 Сумма: <b>${plan.price} ₽</b>\n` +
	// 	`👤 Спонсор: <a href="https://vs-dev-streamer.ru/${sponsor.username}">${sponsor.displayName}</a>\n` +
	// 	`📅 Дата оформления: <b>${new Date().toLocaleDateString()} в ${new Date().toLocaleTimeString()}</b>`,
	// enableTwoFactor:
	// 	`🔐 Обеспечьте свою безопасность!\n\n` +
	// 	`Включите двухфакторную аутентификацию в <a href="https://vs-dev-streamerx.ru/dashboard/settings">настройках аккаунта</a>.`,
	// verifyChannel:
	...
	// 	`Спасибо, что вы с нами и продолжаете развивать свой канал вместе с StreamerX!`
}
